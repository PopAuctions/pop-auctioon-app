...
describe('LanguageSyncEffect', () => {
  ...
  describe('Authenticated user — successful sync', () => {
    it('should call secureGet with CURRENT_USER endpoint on login', async () => {
      mockAuthState = session();
      mockSecureGet.mockResolvedValue({
        data: { id: 'user-123', language: 'en' },
        error: null,
    ...
    it('should call syncLanguageFromDb with "en" when DB value is "en"', async () => {
      mockAuthState = session();
      mockSecureGet.mockResolvedValue({
        data: { id: 'user-123', language: 'en' },
        error: null,
    ...
    it('should call syncLanguageFromDb with "es" when DB value is "es"', async () => {
      mockAuthState = session();
      mockSecureGet.mockResolvedValue({
        data: { id: 'user-123', language: 'es' },
        error: null,
  ...
  describe('Authenticated user — edge cases', () => {
    it('should NOT call syncLanguageFromDb if language field is null/undefined', async () => {
      mockAuthState = session();
      mockSecureGet.mockResolvedValue({
        data: { id: 'user-123', language: null },
        error: null,
    ...
    it('should sync only once per user session (deduplication)', async () => {
      mockAuthState = session('user-abc');
      mockSecureGet.mockResolvedValue({
        data: { id: 'user-abc', language: 'en' },
        error: null,
      ...
      // Re-render simulating parent re-render — should NOT trigger a second fetch
      rerender(<LanguageSyncEffect />);
